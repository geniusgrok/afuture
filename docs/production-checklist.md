...
- [x] 生产跨期/Auto 报单按当前时钟校验新鲜度，每腿发送前复核；限速使用单调时间。
- [x] 生产启动在 process-run/状态/许可写入之前取得账户锁；缺失账户状态不自动初始化。
- [x] 原始账户/持仓查询按 request ID、完整结束标志和身份归集后发布；冲突和 SDK 丢仓拒绝。
- [x] 隔离测试柜台结算格式采集入口已接入实际 CTP adapter；其产物不是资金连续性证明。
- [x] 许可消费后、原状态尚未提交的窄窗口可在同一 lease 与相同新鲜证据下续接；后续人工/风险停机不能复用旧许可。
- [x] 方向性逐笔真实时钟/柜台健康/交易日及今昨仓检查，后续拒单保留已成交事实。
- [x] 已覆盖的运行日历经 RiskManager 接入逐笔开平仓、修复和行情健康检查；不改写冻结研究时段。
- [x] 关键 webhook 队列持久、有界、重试与失败可见；HTTP 接收不等于人工阅读。
- [ ] 目标机原生依赖、实际结算格式、查询顺序与完整性已核验。
- [ ] 正常跨日和阶段授权续接已闭合，不需要每日人工操作。
- [ ] 正式交易日历、夜盘/节假日、数据准备、维护重连与既有风控已形成统一生产路径。
- [ ] 关键通知实际送达级别与独立主机失联检测已经实测。
- [ ] 已批准的隔离测试目标机服务实际安装、启动并保留源码/配置/账户身份收据。
- [ ] 一个真实自然月已完成，交易日、时段、委托成交、恢复和人工介入均有可复核记录。

上面的离线机械项目不代表无人值守整体通过。正常停机不等于账户已经平仓；
工程交付、目标机/测试柜台、真实月份和极小资金授权分别判断，模拟服务不得自动切换真钱。
