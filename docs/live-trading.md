...
配置 `AFUTURE_ALERT_WEBHOOK` 后，CLI 与无柜台能力的 watchdog 复用 `paths.alerts` 同目录的 `.outbox.sqlite3`。事件先写入受限权限、目标地址摘要绑定的 SQLite spool，再由通知线程进行有超时的 HTTPS 发送。不会跟随重定向泄漏事件到未批准地址。默认未解决事件上限 2048 条，单事件上限 16 KiB，数据库上限 32 MiB；HTTP 已接收的近期收据有界保留，重复告警合并计数。
