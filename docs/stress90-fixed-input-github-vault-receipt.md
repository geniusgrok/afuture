...
本收据只记录固定历史研究输入的恢复、兼容回放和私有 GitHub 副本。原始
CSV 未进入本公开仓库。本收据不包含 token、clone credential、signed URL、
原始文件内容或临时路径。
...
- Compatibility checkpoint：`efb44371b0e60c8654d574a86675f8cd339950ce`
...
- 固定历史兼容层只接受五个 basename、size 和 SHA-256 全部精确匹配的输入；
  任一不匹配立即拒绝。
- current-main compatibility replay 与 fixed-archive parity 通过；定向套件
