...
> 2026-08-25 engineering record. For operations, use the current authority documents.
...
- Directional activity sidecar 以 schema/checksum 原子保存 `completed` 与 `in_progress`；Tick 在内存合并，每轮有界事件批次、交易日切换和正常停机时 checkpoint，避免逐 Tick `fsync` 阻塞成交回报。无效文件必须保留用于诊断，并重新观察完整柜台交易日。
...
| 损坏 JSON、checksum/schema/sequence/payload 可在下次 save 时被覆盖 | 不可信恢复状态不得被推进或替换 | `test_save_refuses_to_replace_*`、`test_load_rejects_*` | load/save fail-closed；原文件保持 | 无变化 |
...
| 重启后 CTP 持仓快照已包含成交，但 callback replay 先于去重修改 mirror | 持久成交在 adapter 任何持仓副作用前去重 | engine startup 和 `recover-state` inclusive-snapshot replay regressions | engine、`doctor` 与 `recover-state` 均在 CTP start 前注入已验证复合 identity；不完整 recovery evidence 拒绝采纳 | 无变化 |
...
| `(day, trade_id)` 缺少交易所，可静默吞掉另一交易所的合法同 ID 成交 | 歧义 identity 不能作为 replay 证明 | `test_ambiguous_unqualified_trade_identity_halts_on_owned_cross_exchange_fill` | 只有精确复合键静默去重；不完整 identity 命中时显式 HALT 并要求对账 | 无变化 |
...
行为变化只发生在原先错误或不可信的路径：非法输入现在 fail-closed；bucket 不足不再产生负仓；duplicate fill 在 CTP mirror 和引擎前均不二次入账；交易日不能倒退；跨日成交按正确 bucket 记账；存在 Directional 风险时，非法账户路径先减仓而不是直接 HALT；无效 activity、非权威交易日、歧义 fill identity、未管理交易所持仓、篡改/过期 OHLC 以及删除已验证历史日不再被接受。事件合并只丢弃尚未消费的旧 Tick，缓存只冻结已通过的市场输入，因此合法冻结数据的目标和经济结果不变。
