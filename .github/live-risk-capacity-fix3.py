...
start_marker = '# 8. Final CI includes every explicitly requested stable-candidate gate not already\n'
end_marker = '# 9. Focused verification only.  The PR workflow will perform the single final full\n'
start = text.find(start_marker)
end = text.find(end_marker)
if start < 0 or end < 0 or end <= start:
    raise RuntimeError('runner CI section markers are missing')
text = text[:start] + '# CI workflow augmentation is committed separately with workflow-write authority.\n\n' + text[end:]
