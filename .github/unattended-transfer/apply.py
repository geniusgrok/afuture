...
def main() -> None:
    ...
    require(hashlib.sha256(raw).hexdigest() == '74c71015d909feedf65ed43285bebfbfd4bef2adc4c6b827767d57a970d7ebae', 'manifest changed')
    manifest = json.loads(raw)
    require(manifest['schema_version'] == 1, 'unknown schema')
    require(git('rev-parse', 'HEAD^') == manifest['transport_parent'], 'parent changed')
    chunks = []
    offset = 0
    ...
    git('commit', '-m', 'fix: harden live clocks, startup ownership and CTP evidence')
