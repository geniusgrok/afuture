...
class ExecutionAlignedDirectionalPortfolioManager(DirectionalPortfolioManager):
    ...
    def _next_target_weights(self, history: ExecutionAlignedSignalHistory) -> dict[str, float]:
        ...
        history_kwargs = {}
        if getattr(self.policy, "meta_score_source", None) in HOLDING_SCORE_SOURCES:
            ...
            if dates.empty or dates.isna().any() or dates.max() != last or (dates > last).any():
                raise RuntimeError("specific-contract history must end on the completed signal day")
