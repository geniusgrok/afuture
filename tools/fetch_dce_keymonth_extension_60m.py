"""Coverage-only 60m collector for omitted DCE 01/05/09 key-month products.

The extension is defined by contract lifecycle, not historical strategy outcome. J, JM,
L and V share the same 01/05/09 key-month pattern already used by the frozen M/P/Y/I/PP
60m research surface. This tool never evaluates returns or strategy parameters.
"""
...
ERAS = {
    "prior": {
        ...
        "contracts": ("2209", "2301", "2305", "2309", "2401", "2405", "2409", "2501"),
    },
    "recent": {
        "start": pd.Timestamp("2024-08-21"),
        "end": pd.Timestamp("2026-08-20 23:59:59"),
        "contracts": ("2409", "2501", "2505", "2509", "2601", "2605", "2609", "2701"),
    },
}
...
def collect(era: str) -> tuple[pd.DataFrame, dict]:
    ...
    if frames:
        ...
        data.drop_duplicates(["datetime", "product", "symbol"], keep="last", inplace=True)
    else:
        data = pd.DataFrame(
            columns=["datetime", "open", "high", "low", "close", "volume", "hold", "symbol", "product"]
        )
    report = {
        "role": "DCE 01/05/09 key-month 60m coverage collection only",
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="runtime/dce_keymonth_60m")
    args = parser.parse_args()
    output = Path(args.output_dir)
    output.mkdir(parents=True, exist_ok=True)
    data, report = collect(args.era)
    data.to_csv(output / f"dce_keymonth_{args.era}_60m.csv", index=False)
    (output / f"dce_keymonth_{args.era}_report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    print(json.dumps({"era": args.era, "rows": len(data), "calls": len(report["calls"])}, ensure_ascii=False))
