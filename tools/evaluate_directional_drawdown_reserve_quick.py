"""Five-window quick Production gate for hard-limit-derived drawdown-reserve defense.

The continuous target path is the validated OI-confirmed direction-change + fixed cost
eligibility + tracking-first survivor allocation. This tool replaces only the soft risk
trigger/response: global 0.25x scaling remains disabled, and strict freeze-new-risk is
activated only when completed account drawdown reaches 25% = 30% hard DD - 5% daily-loss
reserve. Hard account constraints are unchanged.
"""
...
REVERSAL_STRESS_REFERENCE = 0.7755524435761105
...
def evaluate(*, specific_raw, continuous_raw, base_weights, bars_60m) -> dict:
    ...
    _confirmed, _approved, candidate_weights = reversal.build_candidate_weights(
        base_weights=base_weights,
        bars_60m=bars_60m,
        close_prices=close,
    )
    base_sim = DrawdownReserveFreezeDirectionalProductionAcceptance(
        ProductionMechanicsConfig(
            initial_capital=mechanics.INITIAL_CAPITAL,
            margin_rate_proxy=mechanics.BASE_MARGIN_PROXY,
        )
    )
    stress_sim = DrawdownReserveFreezeDirectionalProductionAcceptance(
        ProductionMechanicsConfig(
            initial_capital=mechanics.INITIAL_CAPITAL,
    ...
    return {
        "role": "five-window quick Production gate for hard-limit-derived drawdown-reserve freeze",
        "parameter_search": False,
        "production_wiring": False,
        "previous_champion_stress_full_recent": REVERSAL_STRESS_REFERENCE,
        "base_full_recent": base_full,
        "stress": stress,
        "economics": economics,
        "delta_vs_77_reversal_stress_annualized": (
            float(stress["full_recent"]["annualized_return"])
            - REVERSAL_STRESS_REFERENCE
        ),
        "promotion_gate": gate,
        "constraints": {
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"drawdown-reserve quick inputs missing: {missing}")
    report = evaluate(
    ...
    output = runtime / "stress80_drawdown_reserve_quick_report.json"
    output.write_text(
