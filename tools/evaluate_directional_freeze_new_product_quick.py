"""Five-window quick Production gate for freeze-new-product-only defense.

Continuous targets are the validated tracking-first survivor path. The unchanged
DirectionalRiskGovernor trigger remains active, but its soft response blocks only brand-
new product entries; same-product increases, reductions, exits, reversals and rolls remain
executable. All hard account/risk gates are unchanged.
"""
...
def evaluate(*, specific_raw, continuous_raw, base_weights, bars_60m) -> dict:
    ...
    return {
        "role": "five-window quick Production gate for freeze-new-product-only defense",
        ...
        "constraints": {
            ...
            "blocked_when_triggered": "brand_new_product_entries_only",
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"freeze-new-product quick inputs missing: {missing}")
    report = evaluate(
    ...
    output = runtime / "stress80_freeze_new_product_quick_report.json"
    output.write_text(
