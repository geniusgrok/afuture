"""Fixed Production L3 for the 9-product OI overlay with OI-aware selective freeze.

The upstream 9-product Price x OI overlay is frozen and must reproduce the already
validated Production lineage before this candidate is evaluated. The only new behavior
is the soft-defense response: when the unchanged completed-return governor trigger fires,
new/same-sign increases remain frozen for untrusted products, while the nine products
whose targets already passed the causal D -> D+1 OI confirmation may continue to target.
All hard account/risk gates remain unchanged.
"""
...
EXPECTED_OI_BASE = 1.0461362468104771
EXPECTED_OI_STRESS = 0.39609479470641795
OI_LINEAGE_TOLERANCE = 5e-5
CURRENT_CHAMPION_STRESS = 0.68059063
...
def evaluate(
    *,
    ...
    if not all(lineage.values()):
        raise RuntimeError(f"9-product OI Production lineage reproduction failed: {lineage}")

    base_sim = OIAwareFreezeDirectionalProductionAcceptance(
        ProductionMechanicsConfig(
            initial_capital=mechanics.INITIAL_CAPITAL,
            margin_rate_proxy=mechanics.BASE_MARGIN_PROXY,
        )
    )
    stress_sim = OIAwareFreezeDirectionalProductionAcceptance(
        ProductionMechanicsConfig(
            initial_capital=mechanics.INITIAL_CAPITAL,
    ...
    stress_full = float(stress["windows"]["full_recent"]["annualized_return"])
    return {
        "role": "9-product OI confirmation plus OI-aware selective freeze Production L3",
        "production_wiring": False,
        "parameter_search": False,
        "supported_products": list(oi_gate.SUPPORTED_PRODUCTS),
        "oi_only_lineage_reproduction": lineage,
        "oi_only_reference": {
            "workflow_run": 32719520981,
        ...
        "previous_champion_reference": {
            "workflow_run": 32737292428,
            "stress_full_recent": CURRENT_CHAMPION_STRESS,
        },
        "selective_freeze_rule": {
            "completed_daily_loss_trigger": 0.02,
            "completed_two_day_vol_trigger": 0.03,
            "global_point25_scaling": False,
            "trusted_products_bypass_new_risk_freeze": list(oi_gate.SUPPORTED_PRODUCTS),
            "untrusted_new_and_same_sign_increases_freeze": True,
            "reductions_exits_reversals_rolls_bypass": True,
        },
        "candidate": {"base": base, "stress": stress},
        "economics": economics,
        "comparative": {
            "stress_delta_vs_previous_champion": stress_full - CURRENT_CHAMPION_STRESS,
            "beats_previous_champion": stress_full > CURRENT_CHAMPION_STRESS,
        },
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"OI-aware freeze inputs missing: {missing}")
    report = evaluate(
    ...
    output = runtime / "stress80_60m_oi_aware_freeze_report.json"
    output.write_text(
