"""Replay the two frozen C3 crossover sources with the existing account chain."""
import argparse
import hashlib
import json
import sys
...
_specific_d0, _continuous_d0, frozen_base, bars, manifest = pipeline._load_inputs(
    ROOT.parent / 'restored/inputs'
)
D1_HASHES = {
    'return_target_specific_contracts.csv': '96f6156eac519a96661b193d2195278fea80dfd2624f4116abc44f19442ff4f0',
    'broad_daily_universe.csv': 'b3e31794ae6105eb09d859fec1b92cfaf31f86df0d1b67db5b7dde8ccb2786fa',
}
for name, digest in D1_HASHES.items():
    if hashlib.sha256((ROOT.parent / 'D1' / name).read_bytes()).hexdigest() != digest:
        raise ValueError(f'D1 original checksum mismatch: {name}')
specific = pd.read_csv(ROOT.parent / 'D1/return_target_specific_contracts.csv')
continuous = pd.read_csv(ROOT.parent / 'D1/broad_daily_universe.csv')
...
def run(label, exclude_ag):
    ...
    base = ExecutionAlignedAggressivePolicy(
        ...
        specific_contracts=specific, priced_through=closes.index[-1],
    )
    base = base.reindex(columns=STRESS90_POLICY.products, fill_value=0.0)
    cached = ROOT / (label + '_base_weights.csv')
    if cached.exists():
        previous = pd.read_csv(cached, index_col=0, parse_dates=True)
        pd.testing.assert_frame_equal(base, previous, check_exact=False, rtol=0, atol=1e-12)
    ...
    output = {
        ...
        'products': products, 'window': ['2022-09-06', '2026-09-22'],
        'cost_bps': [mechanics.BASE_COST_BPS, mechanics.STRESS_COST_BPS],
        'scenarios': {},
    }
...
if __name__ == '__main__':
    run(opts.label, opts.label.endswith('exAG'))
