"""Cheap roll-safe screen for the parameter-free CZCE receipt-flow alpha."""
...
from afuture.directional_receipt_flow import receipt_flow_weights
...
def receipt_flow_gate(report: Mapping) -> dict:
    ...
    coverage_ok = all(
        float(audit["coverage"]) >= MIN_SOURCE_COVERAGE
    ...
    checks = {
        ...
        "full_recent_net_alpha_per_turnover_positive": float(
            econ["net_alpha_per_turnover_bps"]
        ) > 0.0,
    }
...
def _load(directory: Path) -> tuple[pd.DataFrame, dict]:
    panels: list[pd.DataFrame] = []
    audits: dict[str, dict] = {}
    for window in SOURCE_WINDOWS:
        panel_path = directory / f"receipt_flow_{window}.csv"
        audit_path = directory / f"receipt_flow_{window}_audit.json"
        if not panel_path.exists() or not audit_path.exists():
            raise FileNotFoundError(f"receipt-flow cache incomplete for {window}")
    ...
    frame = pd.concat(panels).sort_index()
    frame = frame[~frame.index.duplicated(keep="last")]
    return frame.apply(pd.to_numeric, errors="coerce"), audits


def build_receipt_flow_weight_history(
    pressure: pd.DataFrame,
    *,
    execution_index: pd.DatetimeIndex,
    products: list[str],
) -> pd.DataFrame:
    targets = pd.DataFrame(0.0, index=pressure.index, columns=products)
    for date, row in pressure.reindex(columns=products).iterrows():
        ...
        for product, weight in receipt_flow_weights(values).items():
            targets.at[date, product] = float(weight)
    return targets.shift(1).reindex(index=execution_index, columns=products).fillna(0.0)


def evaluate_receipt_flow(
    ...
    weights = build_receipt_flow_weight_history(
        pressure,
    ...
    report = {
        "role": "parameter-free CZCE registered-receipt independent alpha screen",
        "production_wiring": False,
        "signal": "-receipt_change / max(abs(today_receipts), abs(yesterday_receipts), 1)",
        "execution": "D completed CZCE receipt total -> D+1 target",
        "weighting": "signed pressure proportional, total gross 2x, no threshold/rank/lookback",
        "supported_exchange": "CZCE",
        ...
        "concentration": {"stress_full_recent_product_net": _concentration(product_net)},
    }
    report["gate"] = receipt_flow_gate(report)
    return report


def main() -> None:
    runtime = Path("runtime")
    pressure, audits = _load(runtime / "receipt_flow")
    report = evaluate_receipt_flow(
        pd.read_csv(runtime / "return_target_specific_contracts.csv"),
        pressure,
        audits,
    )
    output = runtime / "receipt_flow_screen.json"
    output.write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True, default=str),
        encoding="utf-8",
    )
    print(json.dumps({"gate": report["gate"], "stress": {name: report["stress"][name] for name in ("prior1", "prior2", "train", "validation", "oos", "full_recent")}, "economics_full_recent": report["economics"]["stress"]["full_recent"], "coverage": {name: audit["coverage"] for name, audit in report["source_audits"].items()}}, ensure_ascii=False, indent=2))
