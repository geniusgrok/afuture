...
def test_policy_lineage_is_causal_on_last_row():
    ...
    assert (
        baseline_meta.iloc[-1]["selected_templates"] == changed_meta.iloc[-1]["selected_templates"]
    )
    baseline_last = baseline_lineage[baseline_lineage["date"] == baseline.index[-1]].reset_index(
        drop=True
    )
    changed_last = changed_lineage[changed_lineage["date"] == changed.index[-1]].reset_index(
        drop=True
    )
...
def test_exact_lineage_join_reconstructs_realized_position_and_exact_product_cost():
    ...
    product = next(product for product, value in active.items() if abs(float(value)) > 1e-15)
    day = weights.index[-1]
    ...
    row = product_execution[
        (product_execution["date"] == day) & (product_execution["product"] == product)
    ].iloc[0]
...
def test_exact_lineage_carries_position_forward_on_no_trade_days():
    ...
    meta = pd.DataFrame({"date": dates, "selected_templates": [("t1",), ("t1",)]})
    ...
    result = build_exact_lineage_attribution(meta=meta, template_product=lineage, events=events)[
        "product_execution"
    ]
    second = result[(result["date"] == dates[1]) & (result["product"] == "A")].iloc[0]
