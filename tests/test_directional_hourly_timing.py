...
def test_disagreeing_hourly_state_suppresses_new_risk_only():
    from afuture.directional_hourly_timing import apply_hourly_timing_overlay

    dates = pd.to_datetime(["2026-01-05", "2026-01-06", "2026-01-07"])
    raw = pd.DataFrame({"M": [0.0, 1.0, 0.5]}, index=dates)
    state = pd.DataFrame({"M": [0.0, -1.0, -1.0]}, index=dates)

    result = apply_hourly_timing_overlay(raw, state)

    assert result.loc[dates[1], "M"] == 0.0
    # Reduction is always allowed even though the state still disagrees.
    assert result.loc[dates[2], "M"] == 0.5


def test_same_sign_increase_waits_but_exit_and_reversal_bypass():
    from afuture.directional_hourly_timing import apply_hourly_timing_overlay

    dates = pd.to_datetime(["2026-01-05", "2026-01-06", "2026-01-07", "2026-01-08"])
    raw = pd.DataFrame({"M": [1.0, 2.0, 0.0, -1.0]}, index=dates)
    state = pd.DataFrame({"M": [1.0, -1.0, -1.0, 1.0]}, index=dates)

    result = apply_hourly_timing_overlay(raw, state)

    assert result.loc[dates[0], "M"] == 1.0
    assert result.loc[dates[1], "M"] == 1.0
    assert result.loc[dates[2], "M"] == 0.0
    assert result.loc[dates[3], "M"] == -1.0
