...
def test_previous_completed_activity_controls_contract_selection_not_current_ticks(tmp_path):
    ...
    config = DirectionalConfig(enabled=True,products=("A",),exchanges=("DCE",),min_days_to_expiry=20,min_volume=1000,min_open_interest=5000)
    selected = select_contracts_from_activity(config,list(catalog.values()),tracker.completed_snapshot,date(2026, 8, 25))
...
def _snapshot(*, old_volume: float, old_oi: float, new_volume: float, new_oi: float):
    ...
    return DirectionalActivitySnapshot(
        "20260821",
        {
            "A2609": ContractActivity("A2609","DCE","A","20260821",old_volume,old_oi,timestamp),
            "A2611": ContractActivity("A2611","DCE","A","20260821",new_volume,new_oi,timestamp),
        },
    )


def _config():
    return DirectionalConfig(enabled=True,products=("A",),exchanges=("DCE",),min_days_to_expiry=20,min_volume=1000,min_open_interest=5000)
...
def test_roll_hysteresis_cannot_keep_incumbent_inside_delivery_blackout():
    selected = select_contracts_from_activity(
        ...
        date(2026, 9, 1), preferred_symbols={"A": "A2609"},
    )
    assert selected["A"].symbol == "A2611"
