...
def test_sufficient_opposite_depth_uses_best_quote_without_extra_aggression():
    assert depth_aware_opening_price(
...
def test_insufficient_depth_falls_back_to_existing_aggressive_price_and_limits():
    ...
    assert depth_aware_opening_price(
        _tick(ask_volume=4.0, limit_up=101.5), _spec(), OrderSide.BUY, requested_volume=5, aggressive_ticks=2
    ) == 101.5
    assert depth_aware_opening_price(
        _tick(bid_volume=4.0, limit_down=98.5), _spec(), OrderSide.SELL, requested_volume=5, aggressive_ticks=2
    ) == 98.5


def test_invalid_execution_inputs_fail_closed():
    with pytest.raises(ValueError, match="requested_volume"):
        depth_aware_opening_price(_tick(), _spec(), OrderSide.BUY, requested_volume=0, aggressive_ticks=1)
    with pytest.raises(ValueError, match="price_tick"):
        depth_aware_opening_price(
            _tick(), ContractSpec("A2609", "DCE", 10.0, 0.0, 0.1, 0.1),
            OrderSide.BUY, requested_volume=1, aggressive_ticks=1,
        )
