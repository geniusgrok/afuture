    ...
    return pd.DataFrame(
        [
            ...
            },
            {
                ...
                "delta_lots": -10,
                "price": 103.0,
                "turnover_notional": 10300.0,
                "transaction_cost": 15.45,
                "gross_pnl": 0.0,
            },
            {
                "date": "2026-01-08",
                ...
                "delta_lots": 9,
                "price": 104.0,
                "turnover_notional": 9360.0,
                "transaction_cost": 14.04,
                "gross_pnl": 0.0,
            },
        ]
    )
...
def test_future_rows_change_labels_not_completed_history_features():
    ...
    changed = label_directional_entry_exit_events(
        events=_events(),
        specific_contracts=_contracts(),
        product_history=_history(future_scale=2.0),
        horizons=(1, 3, 5),
        stress_cost_bps=15.0,
    )
    ...
    assert baseline.iloc[0]["label_gross_return_h3"] == pytest.approx(0.03)
    assert baseline.iloc[0]["label_net_return_h3"] == pytest.approx(0.027)
    assert baseline.iloc[0]["label_mfe_h3"] == pytest.approx(0.04)
    assert baseline.iloc[0]["label_mae_h3"] == pytest.approx(-0.01)
...
def test_rapid_reentry_is_causal_only_for_the_later_entry():
    ...
    entry = labeled[
        (labeled.event_role == "entry") & (labeled.date == pd.Timestamp("2026-01-08"))
    ].iloc[0]
