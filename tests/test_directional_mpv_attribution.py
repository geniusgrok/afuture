...
def test_completed_product_evidence_uses_only_events_before_decision_day():
    ...
    assert evidence.loc["AG", "lot_segment_exposure"] == 3.0
...
def test_observed_product_evidence_can_fix_intraday_decision_horizon():
    evidence = observed_product_evidence(
        events=sample_events().iloc[:4],
        pnl_actions=("intraday",),
    )
    assert evidence.loc["AG", "gross_pnl"] == 200.0
    assert evidence.loc["AG", "lot_segment_exposure"] == 1.0
    assert evidence.loc["CU", "gross_pnl"] == -50.0
